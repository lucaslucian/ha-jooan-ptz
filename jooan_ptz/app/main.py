...
def validate_camera():
    try:
        ...
        if not isinstance(platform, dict) or "raw" in platform:
            raise ValueError("Camera returned an unexpected getPlatformID response")
        _LOGGER.info("JOOAN camera authentication successful")
        network = None
        try:
